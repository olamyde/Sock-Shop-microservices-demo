pipeline {
    ...
    environment {
        DOCKER_COMPOSE_FILE = "user-contribs/deploy/docker-compose-weave/docker-compose.yml"
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/olamyde/Sock-Shop-microservices-demo.git'
            }
        }
        ...
        stage('Deploy') {
            steps {
                script {
                    // Pull the latest images from DockerHub and deploy
